...
public class AIPANEL extends JLabel implements ActionListener {
	...
	int mousePosY=600/2-panelHeight,level=1;
	int SCORE,AISCORE,DIRECT,acc=rnd.nextInt(level)+1;
	boolean rgb=rnd.nextBoolean();
	int [] panelist=new int[panelHeight];
	AIPANEL(int direct,int level){
		this.level=level;
		DIRECT=direct;
		if(DIRECT==0)
