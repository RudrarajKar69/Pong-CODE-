...
public class MyPanel_KEY extends JLabel implements ActionListener {
	...
	int RpanelY=600/2-panelHeight,LRpanelY=600/2-panelHeight,level=1;
	...
	MyPanel_KEY(int direct,int level){
		this.level=level;
		acc=rnd.nextInt(10)+1;
		DIRECT=direct;
		if(DIRECT==0)
	...
	void racket(Graphics g) {
		...
		g.fillRect(0, RpanelY/*ballY*/, panelWidth, panelHeight);
	}
	...
	void AIracket(Graphics g) {
		...
		g.fillRect(570, LRpanelY/*ballY*/, panelWidth, panelHeight);
	}
...
//				if(panelY!=pos.y) {
//					if(panelY<pos.y) 
//						panelY= panelY+panelSpeed;
//					if(panelY>pos.y)
//						panelY=panelY-panelSpeed;
//					}
//			}
			for(int i=0;i<panelist.length;i++) {
				panelist[i]=RpanelY+i;
			}
			
			for(int i=0;i<AI.length;i++) {
				AI[i]=LRpanelY/*ballY*/+i;
			}	
		}
		repaint();
	}	
